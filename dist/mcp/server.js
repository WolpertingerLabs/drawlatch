 ...
 */
// eslint-disable-next-line @typescript-eslint/no-deprecated -- registerTool is not available in this SDK version
server.tool('poll_events', 'Poll for new events from ingestors (Discord messages, GitHub webhooks, etc.). Returns events received since the given cursor. Pass after_id from the last event you received to get only new events. Omit connection to get events from all ingestors.', {
    connection: z
        .string()
        .optional()
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-deprecated -- registerTool is not available in this SDK version
server.tool('ingestor_status', 'Get the status of all active ingestors for this caller. Shows connection state, buffer sizes, event counts, and any errors.', { _: z.string().optional().describe('unused') }, async () => {
    try {
