...
| `spotify`         | [Spotify Web API](https://developer.spotify.com/documentation/web-api)                      | `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`, `SPOTIFY_REFRESH_TOKEN` | OAuth2 (auto-refresh, see note) |
| `spotify-catalog` | [Spotify Web API](https://developer.spotify.com/documentation/web-api)                      | `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET` | OAuth2 (auto-refresh, see note)    |
...
> **Spotify note:** Spotify has two connection types, both using drawlatch's managed OAuth2 token refresh — the daemon obtains and auto-refreshes the short-lived (1 hour) Bearer access token from the Spotify token endpoint (`https://accounts.spotify.com/api/token`), so you never set a static `SPOTIFY_ACCESS_TOKEN`. Register an app at the [Spotify Developer Dashboard](https://developer.spotify.com/dashboard) to obtain `SPOTIFY_CLIENT_ID` and `SPOTIFY_CLIENT_SECRET`. (1) `spotify` is **user-scoped** (authorization-code + `refresh_token` grant) and covers playback, library, playlists, recently-played, and top tracks (`/v1/me/*`). Mint `SPOTIFY_REFRESH_TOKEN` once via the authorization-code flow with the scopes `user-read-playback-state user-modify-playback-state user-read-currently-playing user-library-read playlist-read-private playlist-read-collaborative user-read-recently-played user-top-read user-read-playback-position`; use the loopback redirect URI `http://127.0.0.1:8080/callback` (`localhost` is not allowed). It includes a poll ingestor on recently-played tracks (default interval 5 minutes; Spotify rate-limits, so keep it generous). (2) `spotify-catalog` is **public** (`client_credentials` grant, no user consent) and covers search + single-item track/album/artist metadata; it needs only the client id/secret. **Feb-2026 Web API caveat:** for Development-Mode apps the batch catalog endpoints (`GET /tracks?ids=`, `/albums?ids=`, etc.) and browse endpoints were removed, and `/search` `limit` is capped at 10 — single-item fetches (`GET /tracks/{id}`) and `/search` still work. Extended-Quota apps are unaffected. The allowlist stays broad (`api.spotify.com/**`).
